...
class DeepPortfolioAgentNetwork(tf.keras.Model):
    def __init__(self, 
                 ...
                 final_dense_units1=128, final_dense_units2=64, final_dropout=0.3, # final_dense_units2 será a dimensão das features latentes
                 use_sentiment_analysis=False,
                 # NOVO: Flag para controlar a saída
                 output_latent_features=False, # Se True, retorna features antes do softmax
                 **kwargs):
        ...
        self.output_latent_features = output_latent_features # ARMAZENA O FLAG

        # ... (definição de self.asset_processor, self.attention, self.attention_norm, self.global_avg_pool_attention como antes) ...
        self.asset_processor = AssetProcessor(...) # Preencha com os args
        calculated_key_dim = self.asset_lstm_output_dim // mha_key_dim_divisor
        if calculated_key_dim == 0: calculated_key_dim = self.asset_lstm_output_dim
        ...
        # ... (código do FinBERT se self.use_sentiment for True) ...
        
        # Camadas Densas que produzem as features latentes
        ...
        self.latent_features_layer = Dense(final_dense_units2, activation='relu', kernel_regularizer=regularizers.l2(L2_REG), name="latent_features_output") # Renomeado para clareza
        self.dropout_latent = Dropout(final_dropout, name="final_dropout_latent") # Dropout após features latentes

        # Camada de alocação (só é adicionada se não estivermos retornando features latentes)
    ...
    def call(self, inputs, training=False):
        ...
        # ... (Processamento individual de ativos com self.asset_processor) ...
        ...
        # ... (Atenção e Agregação) ...
        attention_output = self.attention(query=stacked_asset_features, value=stacked_asset_features, key=stacked_asset_features, training=training)
        ...
        # ... (Lógica de Sentimento se self.use_sentiment) ...
            
        x = self.dense1(current_features_for_dense)
        x = self.dropout1(x, training=training)
        x_latent = self.latent_features_layer(x) # Saída da camada de features latentes
        x_latent_dropout = self.dropout_latent(x_latent, training=training)

        if self.output_latent_features:
            return x_latent_dropout # Retorna as features latentes para SB3
        else:
            # Se chamado para inferência direta (como no seu teste de forward pass)
            portfolio_weights = self.output_allocation(x_latent_dropout) # Aplica a camada de saída final
            return portfolio_weights

    # ... (get_config precisa ser atualizado para incluir output_latent_features e outros params) ...
    def get_config(self): 
        config = super().get_config()
        config.update({
            ...
            # ... todos os outros parâmetros do __init__ ...
            "asset_lstm_output_dim": self.asset_lstm_output_dim, # Embora seja derivado de asset_lstm_units2
            "output_latent_features": self.output_latent_features 
        })
        # Para AssetProcessor, é melhor ele ter seu próprio get_config e from_config,
        # e aqui você pode serializar sua config ou instanciá-lo diretamente no from_config.
        # Por ora, manteremos simples.
        return config

    # @classmethod
    # def from_config(cls, config):
    #    # Precisa de cuidado para recriar submodelos como AssetProcessor
    #    return cls(**config)
...
class DeepPortfolioAgentNetwork(tf.keras.Model):
    def __init__(self, 
                 ...
                 use_sentiment_analysis=False, **kwargs): # Adicionado **kwargs
        ...
        if self.use_sentiment:
            # ... (código do FinBERT como antes) ...
            pass
        
        dense_input_dim = self.asset_lstm_output_dim 
