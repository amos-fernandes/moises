...
# Dados
SYMBOL = 'S&P 500'  # Par de cripto para treinar
TIMEFRAME = '1h'     # Timeframe dos candles ('1m', '5m', '1h', '1d')
# Quantos dados buscar (2 anos de dados de 1h)S
...
def fetch_ohlcv_data_ccxt(symbol, timeframe, days_to_fetch, limit_per_call=1000):
    ...
    exchange = ccxt.binance() # Ou outra exchange ()
    ...
    while True:
        try:
            ...
            # CORRIGIDO: Usar datetime.now(timezone.utc)
            ...
            # Para evitar buscar o mesmo candle, avançamos 1ms do último timestamp
            ...
            # mas uma pausa extra pode ser útil se buscar muitos dados.
...
def calculate_targets(df, horizon=PREDICTION_HORIZON, threshold=PRICE_CHANGE_THRESHOLD):
    ...
    df.dropna(inplace=True) # Remove linhas onde future_price é NaN
    return df
...
def build_lstm_model(input_shape, lstm_units, dense_units, dropout_rate, initial_learning_rate):
    ...
    model = tf.keras.Sequential() # DEFINE O MODELO PRIMEIRO
...
def main():
    ...
    # Mover o índice (timestamp) para uma coluna temporária
    ...
    if not ohlcv_df.index.is_unique:
        ...
        # Inspecionar ohlcv_df.index[ohlcv_df.index.duplicated(keep=False)]
        return
    else:
    ...
    if ta:
        ...
        # Features baseadas em preço (sma_10, macd, bbm)
    ...
    # Único scaler para todas as features (mais simples de gerenciar)
    # Scalers separados (no rnn_predictor.py), precisará adaptar.
    scaler = MinMaxScaler()
    ...
    # Scalers separados em rnn_predictor.py (price_vol_scaler, indicator_scaler),
    # Treinar e salvar esses scalers separadamente aqui.
    # Por agora, este script usa um único scaler para `BASE_FEATURE_COLS`.
    # ADAPTAR ESTA PARTE PARA CORRESPONDER AO `rnn_predictor.py`
    
    # rnn_predictor.py espera PRICE_VOL_SCALER_PATH e INDICATOR_SCALER_PATH:
    # Criar dois DataFrames: um com 'close', 'volume' e outro com 'sma_10', 'rsi_14'
    # Treinar um scaler para cada e salvá-los.
    
    # Salvar price_volume_scaler:
    ...
    # Usar o mesmo scaler que foi usado para criar `scaled_features_df`
    #`scaler.fit_transform(features_to_scale_df)` onde `features_to_scale_df`
    # continha todas as `BASE_FEATURE_COLS`, então `scaler` é o que foi treinado em todas elas.

    # Para salvar scalers separados:
    ...
    # Resultado de vários thresholds indicado e utilizado no rnn_predict
    y_pred_probs = model.predict(X_test)
...
if __name__ == '__main__':
    # Configurar GPU (TensorFlow com suporte a GPU)
...
""" 
    ESTABILIDADE CONQUISTADA! Esta é a maior vitória desta rodada. As curvas de treinamento e validação estão significativamente mais estáveis. O BATCH_SIZE = 64 e o clipvalue=1.0 foram eficazes.

    MELHOR F1-SCORE PARA "RISE" ATÉ AGORA (com threshold 0.50): Um F1-score de 0.43 para a classe minoritária, com um recall de 0.58, é um progresso muito bom. Isso significa que o modelo está identificando mais da metade das oportunidades de subida, e quando ele sinaliza uma subida, tem uma chance razoável (embora não ideal) de estar correto.

    TRADE-OFF PRECISION-RECALL É EVIDENTE: Ao aumentar o threshold, a precisão para "Rise" tende a melhorar, mas o recall cai drasticamente.

    O MODELO ESTÁ APRENDENDO PADRÕES ÚTEIS: Os resultados com threshold 0.50 são um forte indicador de que as features e a arquitetura (com a estabilização) estão permitindo que o modelo capture sinais preditivos. """