...
# --- CRIAÇÃO E WRAPPING DO AMBIENTE ---
print("Criando instância do ambiente PortfolioEnv...")

# 1. Crie uma função lambda que retorna uma nova instância do seu ambiente.
#    Isso é necessário para o DummyVecEnv.
env_creator = lambda: PortfolioEnv(df_multi_asset_features=multi_asset_df, asset_symbols_list=asset_keys_list,
    reward_window_size=REWARD_WINDOW, # Do config 
    risk_free_rate_per_step=frisk_free_per_step) 

    # 2. Envolva a função criadora com DummyVecEnv.
#    SB3 trabalha com ambientes "vetorizados", mesmo que seja apenas um.
print("Envolvendo o ambiente com DummyVecEnv...")
env = DummyVecEnv([env_creator])

# 3. Envolva o DummyVecEnv com VecNormalize. ESTE É O PASSO PRINCIPAL!
print("Envolvendo o ambiente com VecNormalize (normalizando observações e recompensas)...")
# Definir o caminho para salvar as estatísticas do VecNormalize
stats_path = os.path.join("app/model_vec/", "vec_normalize_stats.pkl") # Use o MODEL_SAVE_DIR do seu config


# Verificar se um arquivo de estatísticas já existe para carregar (para avaliação/continuação)
if os.path.exists(stats_path):
    print(f"Carregando estatísticas de normalização existentes de: {stats_path}")
    env = VecNormalize.load(stats_path, env)
    # Ao carregar para continuar o treinamento, deixe training=True.
    # Para avaliação, você setaria env.training = False e env.norm_reward = False.
    env.training = True
    env.norm_reward = True
else:
    # Se não existir, crie um novo wrapper VecNormalize.
    print("Nenhum arquivo de estatísticas encontrado. Criando novo VecNormalize.")
    env = VecNormalize(
        env, 
        norm_obs=True,         # Normaliza as observações (muito recomendado)
        norm_reward=True,      # Normaliza as recompensas (muito recomendado)
        clip_obs=10.0,         # Limita o valor das observações (evita valores extremos)
        clip_reward=10.0,      # Limita o valor das recompensas
        gamma=0.99,            # Deve ser o mesmo gamma que você usará no PPO
        training=True          # Importante: True para treinamento, False para avaliação
    )

# --- O resto do seu script ---
print("Ambiente de Portfólio Vetorizado e Normalizado Criado.")
...
model_ppo.save("app/model_vec/ppo_custom_deep_portfolio_agent")
print("Modelo RL com política customizada treinado e salvo.")

model_ppo.save("app/model_vec/model3")
print("Modelo RL com política customizada treinado e salvo.")

model_ppo.save("app/model_vec/model2.h5")
print("Modelo RL com política customizada treinado e salvo.")

MODEL_SAVE_DIR="app/model_vec/"
RL_AGENT_MODEL_NAME="model_vec.h5"

print("Salvando modelo RL treinado...")
model_ppo.save(os.path.join(MODEL_SAVE_DIR, RL_AGENT_MODEL_NAME)) # Use nomes do config

print(f"Salvando estatísticas de normalização em: {stats_path}")
env.save(stats_path) # SALVA as médias e desvios padrão calculados durante o treino

print("Modelo e estatísticas de normalização salvos.")
#----------
